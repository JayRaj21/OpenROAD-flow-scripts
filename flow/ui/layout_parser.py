...
LAYER_COLORS = {
    # NanGate45
    'metal1': '#4488ff', 'metal2': '#ff4444', 'metal3': '#44bb44',
    'metal4': '#ff8800', 'metal5': '#aa44ff', 'metal6': '#00cccc',
    'metal7': '#ff44cc', 'metal8': '#888800', 'metal9': '#008888',
    'metal10': '#884400',
    # sky130
    'li1': '#ffcc44',
    'met1': '#4488ff', 'met2': '#ff4444', 'met3': '#44bb44',
    'met4': '#ff8800', 'met5': '#aa44ff',
    # asap7
    'm1': '#4488ff', 'm2': '#ff4444', 'm3': '#44bb44',
    'm4': '#ff8800', 'm5': '#aa44ff', 'm6': '#00cccc',
    'm7': '#ff44cc', 'm8': '#888800', 'm9': '#008888',
}
...
def find_lef_files(platform, flow_dir):
    ...
    lefs = (
        glob(os.path.join(platform_dir, 'lef', '*.lef')) +
        glob(os.path.join(platform_dir, '*.lef'))
    )
...
def _parse_route_segments(text):
    ...
    for chunk in re.split(r'\+\s+ROUTED\b|\bNEW\b', text):
        ...
        if layer in {'use', 'shape', 'style', 'source', 'net', 'taper',
                     'noshape', 'virtual', 'fix', 'cover', '+', 'end'}:
            continue
        width = int(m.group(2))
        segs = re.findall(
                ...
                for layer, width, coords in _parse_route_segments(stmt):
                    ...
                    for x1, y1, x2, y2 in coords:
                        result['specialRoutes'].append(
                            {'layer': layer, 'x1': x1, 'y1': y1,
                             'x2': x2, 'y2': y2, 'w': width}
                        )

            # ── Signal routing ──────────────────────────────────────────────
            elif section == 'nets' and stmt.startswith('- ') and not result['routesTruncated']:
                for layer, width, coords in _parse_route_segments(stmt):
                    layers_seen.add(layer)
                    for x1, y1, x2, y2 in coords:
