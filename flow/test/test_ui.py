...
class TestGetDesignNickname(unittest.TestCase):
    ...
    def test_explicit_nickname_question_equals(self):
        self._make_config('nangate45', 'black_parrot',
                          'export DESIGN_NICKNAME ?= bp\n')
        self.assertEqual(
            layout_parser.get_design_nickname('nangate45', 'black_parrot', self.tmp.name),
            'bp')

    def test_explicit_nickname_plain_equals(self):
        self._make_config('nangate45', 'gcd', 'export DESIGN_NICKNAME = gcd\n')
        self.assertEqual(
            layout_parser.get_design_nickname('nangate45', 'gcd', self.tmp.name),
            'gcd')
...
class TestParseTimingReports(unittest.TestCase):
    ...
    def test_nickname_used_for_path(self):
        ...
        result = layout_parser.parse_timing_reports('nangate45', 'black_parrot', self.tmp.name)
        self.assertIn('Final', result['stages'])
...
class TestParseDef(unittest.TestCase):
    ...
    def test_result_is_cached(self):
        path = self._write_def('UNITS DISTANCE MICRONS 1000 ;\nDIEAREA ( 0 0 ) ( 1000 1000 ) ;\n')
